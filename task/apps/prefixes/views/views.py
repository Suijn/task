...
class PrefixViewSet(viewsets.ViewSet):
    @extend_schema(
        responses={200: PrefixSerializerOut},
    )
    def list(self, request):
        queryset = Prefix.objects.all().prefetch_related("items")
        serializer = PrefixSerializerOut(queryset, many=True)
    ...
    def retrieve(self, request, pk: int | None = None):
        ...
        serializer = PrefixSerializerOut(prefix)
        return Response(serializer.data)

    @extend_schema(
        request=PrefixSerializerIn,
        responses={201: PrefixSerializerOutOnlyName, 409: ConflictSerializerOut},
    )
    def create(self, request):
        data_in = request.data
        log = logger.bind(data_in=data_in)

        serializer = PrefixSerializerIn(data=data_in)
        ...
        try:
            ...
            serializer = PrefixSerializerOutOnlyName(prefix)
